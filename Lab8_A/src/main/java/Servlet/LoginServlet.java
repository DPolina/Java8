...
public class LoginServlet extends ChatServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse
        ...
        // Вывести форму
        pw.println("<form action='/Lab8_A/' method='post'> Enter a name: <input type='text' name='name' value=''><input type='submit' value='Enter a chat'>");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse
        ...
        if (errorMessage!=null) {
            ...
            response.sendRedirect(response.encodeRedirectURL("/Lab8_A/"));
        }
    }
    ...
    String processLogonAttempt(String name, HttpServletRequest request,
        ...
        if (aUser.getSessionId().equals(sessionId) ||
            ...
            response.sendRedirect(response.encodeRedirectURL("/Lab8_A/view.htm"));
            messages.add(new ChatMessage((String) name + " entered the chat", 
        			new ChatUser("System", Calendar.getInstance().getTimeInMillis(), 
        					request.getSession().getId()), Calendar.getInstance().getTimeInMillis()));
            ...
            return "Sorry, but name  <strong>" + name + "</strong> is already in use. Please, choose another name!";
        }
    }
}
